...
import { format, addMonths, getDay, isWithinInterval, addDays } from "date-fns"
import { Calendar, Clock, Plus, Edit, Trash2, RefreshCw, CalendarDays } from "lucide-react"
...
export function AvailabilityManagementDialog({
  isOpen,
  onClose,
  ...
  const [dateRange, setDateRange] = useState<Date[] | undefined>([selectedDate])
  const [calendarMonth, setCalendarMonth] = useState(selectedDate)

  // Set the initial tab based on props
  useEffect(() => {
    setActiveTab(initialTab)
  }, [initialTab])

  // Update the selected date when the prop changes
  useEffect(() => {
    setDate(selectedDate)

    // Also update the new exception form with the selected date
  ...
  const markedDays = exceptionDates
  ...
  // Check if the selected date has any exceptions
  const selectedDateHasExceptions = date
    ? exceptions.some((ex) => {
        if (ex.type === "single" || ex.type === "range") {
          const start = new Date(ex.startDate)
          const end = new Date(ex.endDate)
          ...
          return isDateInRecurringPattern(date, ex)
        }
        return false
      })
    : false

  // If a date is selected and has exceptions, default to the exceptions tab
  useEffect(() => {
    if (selectedDateHasExceptions) {
      setActiveTab("exceptions")
    }
  }, [selectedDateHasExceptions])
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Set your availability for sessions and manage exceptions</DialogDescription>
        </DialogHeader>
