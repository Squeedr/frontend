...
export default function AuthPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "register") {
        // Register user with role - backend will assign the role
        const data = await authApi.register({ username, email, password, selectedRole: role });
        toast({ title: "Registered!", description: `Welcome, ${data.user.username}` });
        
        // Process the registration response using the role context
        // The backend should now return user with app_roles assigned
        await processLoginResponse(data);
      } else {
